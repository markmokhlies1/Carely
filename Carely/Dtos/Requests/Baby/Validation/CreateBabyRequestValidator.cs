...
namespace Carely.Dtos.Requests.Baby.Validation
{
    ...
    {
       public CreateBabyRequestValidator() {
            ...
            // .GreaterThanOrEqualTo(DateTime.Today.AddYears(-3)).WithMessage("Age cannot exceed 3 years.");
            RuleFor(x => x.DateOfBirth)
             .NotEmpty().WithMessage("Date of Birth is required")
             .Must(d => d.ToUniversalTime().Date <= DateTime.UtcNow.Date)
                 .WithMessage("Date of Birth cannot be in the future")
             .Must(d => d.ToUniversalTime().Date >= DateTime.UtcNow.Date.AddYears(-3))
                 .WithMessage("Age cannot exceed 3 years.");
